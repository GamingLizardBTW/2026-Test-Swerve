...
sw_data = {
    ...
    # Setpoint 
    "FirstMotorSetpoint": 30.0,     #number of rotations to move to

    #Second motor PID
    "Second_ks": 0.2,     # Static friction
    "Second_kv": 0.12,    # Velocity feedforward
    "Second_ka": 0.0,     # Acceleration feedforward raise if system needs to be faster

    "Second_kp": 3.0,     # Too high and will cause oscillation, too low will make it not reach desired place
    "Second_ki": 0.5,     # Helps fix small errors over time 
    "Second_kd": 0.1,     # Stabalizing, fixes over shoot

    # Motion Magic
    "Second_Cruise_Velocity": 40,   # rotations/sec   Max Speed
    "Second_Acceleration": 80,      # rotations/sec^2 How fast it can accelerate
    "Second_Jerk": 0,               # optional        First push to start acceleration

    "Second_Gear_Ratio": 5.0,       #change if motor has gearing

    # Setpoint 
    "SecondMotorSetpoint": 30.0,     #number of rotations to move to
}
